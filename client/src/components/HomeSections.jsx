...
export default function HomeSections({ posts }) {
  const features = [
    ...
    { icon: Lock, title: 'SSL/TLS AUDIT', desc: 'Certificate validity, cipher suites, protocol versions, HSTS' },
    { icon: Terminal, title: 'PORT DETECTION', desc: 'Identify open ports and exposed services passively' },
    { icon: FileCode, title: 'SECURITY HEADERS', desc: 'Check CSP, X-Frame-Options, HSTS, and referrer policy' },
    { icon: ShieldAlert, title: 'IP REPUTATION', desc: 'Cross-reference against 50+ threat databases and blocklists' },
    { icon: Zap, title: 'AI RISK REPORT', desc: 'Plain-English risk summary generated from all scan data' },
  ]
  ...
  const stats = [
    { val: '1.2M+', label: 'ACTIVE THREATS' },
    { val: '84.2K', label: 'NODES MONITORED' },
    { val: '12.5K', label: 'DAILY SCANS' },
    { val: '2,500+', label: 'RESEARCHERS' },
  ]
    ...
    <div className="space-y-32 py-20">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-4">
          {steps.map((s, i) => (
            <React.Fragment key={i}>
              <div className="flex-1 bg-[#0d1117] border border-[#1a2332] p-6 rounded-[6px] w-full">
                <span className="block font-mono text-lg text-[#00ff88] mb-4">{s.num}</span>
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block text-[#1a2332] text-2xl px-4 animate-pulse">▸</div>
              )}
            </React.Fragment>
          ))}
        </div>
      ...
      <section>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((s, i) => (
            <div key={i} className="bg-[#0d1117] border border-[#1a2332] p-6 rounded-[6px] text-center">
              <div className="text-2xl font-bold text-[#00ff88] mb-2">{s.val}</div>
              <div className="font-mono text-[10px] text-[#475569] uppercase tracking-[2px]">{s.label}</div>
            </div>
          ))}
        </div>
        <p className="text-center text-[12px] text-[#475569] font-sans">
          Trusted by security researchers, SOC analysts, and developers worldwide.
        </p>
      </section>
