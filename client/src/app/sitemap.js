...
async function getBlogPosts() {
  const query = `*[_type == "post" && defined(slug.current) && !(_id in path("drafts.**"))] | order(coalesce(_updatedAt, publishedAt) desc) { "slug": slug.current, publishedAt, _updatedAt }`
  ...
  const staticUrls = [
    {
      ...
      changeFrequency: 'weekly', // 'daily' is usually overkill unless homepage changes every day
      priority: 1.0,
    },
    {
      url: `${BASE_URL}/blog`,
