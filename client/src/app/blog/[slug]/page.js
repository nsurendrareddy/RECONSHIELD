  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/blog/${slug}`)
      ...
      .then(data => {
        setPost(data)
        setLoading(false)
        // Update title for client-side
        document.title = `${data.title} | ReconShield Blog`;
      })
      .catch(err => {
        console.error(err)
        setError(true)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 space-y-4">
        <div className="w-10 h-10 border-2 border-matrix-400/20 border-t-matrix-400 rounded-full animate-spin" />
        <div className="text-gray-500 font-mono text-xs uppercase tracking-widest">Decrypting intelligence briefing...</div>
      </div>
    )
  }
